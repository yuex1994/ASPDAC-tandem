...
void encrypt_128_key_expand_inline_no_branch_ila(word state[], word key[]) {
    int nr = 10;
    int i;
...
uint128_t AES::aes128(uint128_t arg_0, uint128_t arg_1) {
  ...
  encrypt_128_key_expand_inline_no_branch_ila(output, key);
  uint128_t result;
  for (int i = 0; i < 4; i++) {
    for (int j = 0; j < 4; j++) {
...
uint2_t AES::unknown0() {
  return 0;
}

uint2_t AES::unknown1() {
  return 0;
}

uint128_t AES::unknown2() {
  return 0;
}