...
template <typename axiCfg, bool enable_interrupts = false> class MasterFromFile : public sc_module {
 public:
  ...
  MasterFromFile(sc_module_name name_, std::string filename="requests.csv", int impl_jump_start_instr=0)
    ...
    for (unsigned int i=impl_jump_start_instr; i < dataList.size(); i++) {
