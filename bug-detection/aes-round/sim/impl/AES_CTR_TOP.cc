...
void AES_block_encrypt(uint32_t key[4], uint32_t plaintext[4], uint32_t output[4])
{
    ...
    for (int i = 0; i < 4; i++) {
        for (int j = 0; j < 4; j++) {
        uint32_t tmp = (uint32_t) ((output[i] >> (j * 8)) & mask);
        in_0[3 - i] += (tmp << ((3 - j) * 8)); 
        // std::cout << "tmp[" << i << ", " << j << "] = " << tmp << std::endl; 
        tmp = (uint32_t) ((key[i] >> (j * 8)) & mask);
        in_1[3 - i] += (tmp << ((3 - j) * 8));
        }
        // std::cout << "output[" << 3 - i << "] = " << output[3 - i] << std::endl;
    }       
    encrypt_128_key_expand_inline_no_branch(in_0,in_1);    
  for (int i = 0; i < 4; i++) {
    uint32_t result = 0;
    for (int j = 0; j < 4; j++) {
      uint32_t tmp = in_0[i];
...
void AES_CTR_start(struct AES_parameter param)
{
    ...
    for( byteCount = 0; byteCount < param.EncryptLength;  byteCount += 16)
    {
        ...
        // std::cout << "param" << param.PlainTextPointer << std::endl;
        // std::cout << "addr" << param.PlainTextPointer + byteCount/4 << std::endl;
        ...
        // std::cout << "ctr0: " << std::hex << AES_ctr[0] << std::endl;
        // std::cout << "ctr1: " << std::hex << AES_ctr[1] << std::endl;
        // std::cout << "ctr2: " << std::hex << AES_ctr[2] << std::endl;
        // std::cout << "ctr3: " << std::hex << AES_ctr[3] << std::endl;

        // std::cout << "key0: " << std::hex << key[0]  << std::endl;
        // std::cout << "key1: " << std::hex << key[1]  << std::endl;  
        // std::cout << "key2: " << std::hex << key[2]  << std::endl;  
        // std::cout << "key3: " << std::hex << key[3]  << std::endl;  
        // std::cout << "plaintex0: " << std::hex << plaintext[0]  << std::endl;
        // std::cout << "plaintex1: " << std::hex << plaintext[1]  << std::endl;  
        // std::cout << "plaintex2: " << std::hex << plaintext[2]  << std::endl;  
        // std::cout << "plaintex3: " << std::hex << plaintext[3]  << std::endl;         
        c_xor(tmp_result,plaintext,final_result,4);
        // std::cout << "plaintext: " << plaintext[0] << std::endl;
