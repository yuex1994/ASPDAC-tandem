...
void ExportTandemSimChecker(const Ila& ila, const std::string& rtl_path, 
                      ...
                      const std::string& out_path, bool opt, bool aux_pc) {
  TandemGenerator tg(ila.get());
  tg.SetRTLInfo(rtl_path, rtl_top);
  tg.SetRefMap(ref_map);
  tg.GenerateS2(out_path, opt, aux_pc);
}

void ExportTandemSimConverter(const Ila& ila, const std::string& rtl_path, 
                      const std::string& rtl_top, 
                      const std::string& ref_map,
                      const std::string& out_path, bool opt, bool aux_pc) {
  TandemGenerator tg(ila.get());
  tg.SetRTLInfo(rtl_path, rtl_top);
  tg.SetRefMap(ref_map);
  tg.GenerateS4(out_path, opt, aux_pc);
}
