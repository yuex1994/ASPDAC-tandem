...
void Ilator::DfsConst(const ExprPtr& expr, StrBuff& buff, ExprVarMap& lut) {
  ...
  fmt::format_to(buff, kConstNonMemTemplate, //
                 fmt::arg("var_type", GetIlatedType(expr)),
                 fmt::arg("local_var", local_var),
...
void Ilator::DfsOpSpecial(const ExprPtr& expr, StrBuff& buff, ExprVarMap& lut) {
  ...
  case AstUidExprOp::kConcatenate: {
    ...
    if (cxx_en_) {      
      static const char* kConcatTemplate =
          "{global_var} = "
          "(boost::numeric_cast<{var_type}>({arg_0}) << {shift_w}) + "
          "boost::numeric_cast<{var_type}>({arg_1});\n";
      auto arg0 = expr->arg(0);
      auto arg1 = expr->arg(1);
      auto shift_w = (arg1->is_bool()) ? 1 : (arg1->sort()->bit_width());
      ...
      fmt::format_to(buff, kConcatTemplate, //
                     fmt::arg("global_var", global_var),
                     fmt::arg("type_0", GetIlatedType(arg0)),
                     fmt::arg("arg_0", LookUp(arg0, lut)),
                     fmt::arg("type_1", GetIlatedType(arg1)),
                     fmt::arg("arg_1", LookUp(arg1, lut)));
    }
    break;
  }
  case AstUidExprOp::kExtract: {
    if (cxx_en_) {
      static const char* kExtractTemplate =
          "auto {extract}_shifted = {origin} >> {loc_low};\n"
          "auto {extract}_mask = {origin};\n"
          "{extract}_mask = (1 << ({loc_high} - {loc_low} + 1)) - 1;\n"
          "{extract}_mask &= {extract}_shifted;\n"
          "{var_type} {extract} = boost::numeric_cast<{var_type}>({extract}_mask);\n";
  ...
  case AstUidExprOp::kZeroExtend:
    if (cxx_en_) {
      static const char* kExtendTemplate =
          "{ext_type} {extend} = "
          "boost::numeric_cast<{ext_type}>({origin});\n";
      ...
      fmt::format_to(buff, kExtendTemplate, //
                     ...
                     fmt::arg("ext_type", GetIlatedType(expr)));
      break;
    } else {
      [[fallthrough]];
    }
  case AstUidExprOp::kSignedExtend: {
    if (cxx_en_) {
      static const char* kExtendTemplate =
          "{ext_type} {extend} = "
          "boost::numeric_cast<{ext_type}>"
          "((({origin} >> {origin_shift}) == 1) ? (~{origin}) : {origin});\n"
          "{extend} = (({origin} >> {origin_shift}) == 1) ? (~{extend}) : {extend};\n";
      auto origin_expr = expr->arg(0);
      fmt::format_to(buff, kExtendTemplate, //
                     fmt::arg("extend", local_var),
                     fmt::arg("origin", LookUp(origin_expr, lut)),
                     fmt::arg("origin_shift", origin_expr->sort()->bit_width() - 1),
                     fmt::arg("ext_type", GetIlatedType(expr)));
    } else {
      static const char* kExtendTemplate =
