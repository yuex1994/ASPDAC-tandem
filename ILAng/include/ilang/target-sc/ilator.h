...
public:
  ...
  Ilator(const InstrLvlAbsPtr& m, TandemGenerator* t);
  /// Destructor.
  ~Ilator();
  ...
  void GenerateCXX(const std::string& dst, bool opt);
  void Generate(const std::string& dst, bool opt);
  void EnableTandemChecker(TandemGenerator* t) { 
...
private:
  ...
  /// Extra used type;
  static std::set<int> cxx_var_width_;
  ...
  static inline std::string GetIlatedType(const ExprPtr& expr) {
    return GetCxxType(expr->sort());
  }
  /// Get the type of sort in SystemC.
  static std::string GetIlatedType(const SortPtr& sort);
  static std::string GetCxxType(const SortPtr& sort);
  static std::string GetSysCType(const SortPtr& sort);
